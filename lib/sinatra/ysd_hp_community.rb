    ...
    def render_profile_action_button(option)
      ...
      if option[:data_icon]
       profile_button = <<-PROFILE_BUTTON
         <div id="#{option[:id]}" class="form-button #{option[:class]}"><a href="#{option[:link]}" data-icon=#{option[:data_icon]}></a></div>
         <span class="smaller_text">#{option[:text]}</span>
       PROFILE_BUTTON
      else
       profile_button = <<-PROFILE_BUTTON
          <div id="#{option[:id]}" class="form-button #{option[:class]}"><a href="#{option[:link]}">#{option[:text]}</a></div>
       PROFILE_BUTTON
      end
